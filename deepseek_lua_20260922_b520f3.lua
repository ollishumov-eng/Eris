...
local BG_IMAGE = "rbxassetid://95080673394037"
...
local S = {
    ...
    kick = false, kickTask = nil,
    kickMethod = "Kick 1",
    rogdoll = false, rogdollTask = nil,
    rogdollMethod = "Pallet Rogdoll",
...
-- ============ KICK 1 ============
local function kick1(targetName)
    ...
    local function currentLock()
        local curRoot = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
        if curRoot then
            return curRoot.CFrame * CFrame.new(0, 17, 0)
        end
        return saved * CFrame.new(0, 17, 0)
    end
    ...
    while S.kick do
        ...
        if tr and th and th.Health > 0 and root then
            if not dragging then
                ...
                local grabTime = (reGrabCD > tick() - 1) and 0.15 or 0.25
                if grabT == 0 then grabT = tick() end
                if tick() - grabT > grabTime then
                    dragging = true
                    grabT = 0
                    checkT = tick()
                    createBodies(tr, currentLock().Position)
                end
            else
                local lock = currentLock()
                ...
                if burstLeft > 0 then
                    ...
                    n = fps > 200 and 2 or (fps >= 155 and 3 or 4)
                end
                ...
                if tick() - burstCD > 0.5 then
                    burstCD = tick()
                    burstLeft = 6
                end

                if checkT > 0 and tick() - checkT > 0.2 then
                    if (tr.Position - lock.Position).Magnitude > 15 then
                        dragging = false
                        grabT = 0
                        checkT = 0
                        reGrabCD = tick()
                        clear()
                        if (tr.Position - root.Position).Magnitude > 25 then
                            root.CFrame = tr.CFrame * CFrame.new(0, 0, 3)
                        end
                    else
                        checkT = tick()
...
-- ============ HIT RAGDOLL (ты остаёшься на месте) ============
local function hitRagdoll(targetName)
    local t = Players:FindFirstChild(targetName)
    ...
    while S.rogdoll do
        ...
        if not th or not tr or th.Health <= 0 then task.wait(0.5); continue end

        local myRoot = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
        if not myRoot then task.wait(0.5); continue end

        -- ЗАПОМИНАЕМ МОЮ ПОЗИЦИЮ - я никуда не двигаюсь
        local myPos = myRoot.CFrame

        -- поднимаем цель над собой на 17 studs
        local lock = myPos * CFrame.new(0, 17, 0)
        for i = 1, 4 do
            pcall(function() GE.SetNetworkOwner:FireServer(tr, lock) end)
        end
        tr.CFrame = lock
        pcall(function()
            th.PlatformStand = true
        ...
        -- держим 2.5 секунды сверху
        local holdStart = tick()
        while tick() - holdStart < 2.5 and S.rogdoll do
            local curRoot = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
            if curRoot then
                -- обновляем lock если я двигаюсь (но по факту я стою)
                local lk = curRoot.CFrame * CFrame.new(0, 17, 0)
                for i = 1, 3 do
                    pcall(function() GE.SetNetworkOwner:FireServer(tr, lk) end)
                end
                tr.CFrame = lk
                pcall(function()
                    th.PlatformStand = true
        ...
        if not S.rogdoll then break end

        -- УДАР О ЗЕМЛЮ: цель падает вниз, я стою на своём месте
        local curRoot = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
        if not curRoot then task.wait(0.5); continue end
        local downCF = CFrame.new(curRoot.Position.X, curRoot.Position.Y - 60, curRoot.Position.Z)
        for i = 1, 6 do
            pcall(function()
                GE.SetNetworkOwner:FireServer(tr, downCF)
                GE.DestroyGrabLine:FireServer(tr)
            end)
        end
        tr.CFrame = downCF
        tr.AssemblyLinearVelocity = Vector3.new(0, -500, 0)
        ...
        pcall(function()
            if th:FindFirstChild("Ragdolled") then
                ragdolled = th.Ragdolled.Value
            else
                if th:GetState() == Enum.HumanoidStateType.Physics then
                    ragdolled = true
                end
            end
        end)
        ...
        if not ragdolled then
            ...
            tr.AssemblyLinearVelocity = Vector3.new(0, -800, 0)
            tr.AssemblyAngularVelocity = Vector3.new(
                math.random(-400, 400),
                math.random(-400, 400),
                math.random(-400, 400)
        ...
        -- возвращаем цель наверх, ко мне (я всё ещё на месте)
        task.wait(0.1)
        local curRoot2 = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
        if curRoot2 then
            local upCF = curRoot2.CFrame * CFrame.new(0, 17, 0)
            for i = 1, 5 do
                pcall(function() GE.SetNetworkOwner:FireServer(tr, upCF) end)
            end
            tr.CFrame = upCF
        end
        ...
        while tick() < waitUntil and S.rogdoll do
            -- держим цель сверху, я на месте
            local curRoot3 = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
            if curRoot3 and tr.Parent then
                local holdLock = curRoot3.CFrame * CFrame.new(0, 17, 0)
                pcall(function() GE.SetNetworkOwner:FireServer(tr, holdLock) end)
                pcall(function() th.PlatformStand = true end)
            end
            task.wait()
        end
    end
...
local function snowballRagdollOld(targetName)
    local t = Players:FindFirstChild(targetName)
    if not t then return end
    local spawn = REP:WaitForChild("MenuToys"):WaitForChild("SpawnToyRemoteFunction")
    while S.snowball do
...
local MOBILE_W, MOBILE_H = 380, 340
local PC_W, PC_H = 720, 720

local mainSize = IS_MOBILE and UDim2.new(0, MOBILE_W, 0, MOBILE_H) or UDim2.new(0, PC_W, 0, PC_H)
...
local TOPH = IS_MOBILE and 46 or 75
...
local title = label(top, "Eris hub", IS_MOBILE and 20 or 34, Color3.fromRGB(255,255,255))
title.Size = UDim2.new(1, -20, 1, 0)
title.Position = UDim2.new(0, IS_MOBILE and 10 or 20, 0, 0)
title.ZIndex = 4
...
local SIDEBAR = IS_MOBILE and 110 or 190
...
sl.Padding = UDim.new(0, IS_MOBILE and 4 or 8)
sl.SortOrder = Enum.SortOrder.LayoutOrder
local sp = Instance.new("UIPadding", sideScroll)
sp.PaddingTop = UDim.new(0, IS_MOBILE and 6 or 12)
sp.PaddingLeft = UDim.new(0, IS_MOBILE and 6 or 12)
sp.PaddingRight = UDim.new(0, IS_MOBILE and 6 or 12)
sp.PaddingBottom = UDim.new(0, IS_MOBILE and 6 or 12)
...
content.Size = UDim2.new(0.96, 0, 1, 0)
content.Position = UDim2.new(0.5, 0, 0, 0)
content.AnchorPoint = Vector2.new(0.5, 0)
content.BackgroundTransparency = 1
content.ZIndex = 2
local cp = Instance.new("UIPadding", content)
cp.PaddingTop = UDim.new(0, IS_MOBILE and 6 or 10)
cp.PaddingLeft = UDim.new(0, IS_MOBILE and 4 or 8)
cp.PaddingRight = UDim.new(0, IS_MOBILE and 4 or 8)
cp.PaddingBottom = UDim.new(0, IS_MOBILE and 6 or 10)
...
local function makeGroup(p, title, order)
    ...
    local h = label(wrap, title, IS_MOBILE and 12 or 14, ACCENT2)
    ...
    local function fit()
        ...
        local total = contentY + 26
        if total < 38 then total = 38 end
...
local function createSection(name, icon, order)
    ...
    b.Name = name .. "Btn"
    b.Size = UDim2.new(1, 0, 0, IS_MOBILE and 34 or 48)
    ...
    ic.Size = UDim2.new(0, IS_MOBILE and 22 or 30, 0, IS_MOBILE and 22 or 30)
    ...
    local l = label(b, name, IS_MOBILE and 11 or 15, Color3.fromRGB(200, 215, 230))
    l.Name = "Label"
    l.Size = UDim2.new(1, -34, 1, 0)
    l.Position = UDim2.new(0, IS_MOBILE and 32 or 46, 0, 0)
    l.ZIndex = 5
...
do
    ...
    local welcome = label(p, "Welcome to Eris Hub", IS_MOBILE and 16 or 22, Color3.fromRGB(255,255,255))
    welcome.Size = UDim2.new(1, 0, 0, IS_MOBILE and 22 or 30)
    ...
    text.Size = UDim2.new(1, 0, 0, IS_MOBILE and 90 or 110)
    text.BackgroundTransparency = 1
    text.Text = "Thank you for launching my script; I hope it lives up to its price and your expectations. Wishing you a great mood and an enjoyable game with my script. By Marlin."
    text.TextColor3 = Color3.fromRGB(215, 225, 235)
    text.TextSize = IS_MOBILE and 13 or 16
    ...
    local discordRow = Instance.new("Frame", p)
    discordRow.Size = UDim2.new(1, 0, 0, 30)
    discordRow.BackgroundColor3 = Color3.fromRGB(8, 12, 18)
    discordRow.BackgroundTransparency = 0.35
    discordRow.BorderSizePixel = 0
    discordRow.LayoutOrder = 3
    corner(discordRow, 10)

    local discordLbl = label(discordRow, "Discord server —", IS_MOBILE and 12 or 14, Color3.fromRGB(220, 230, 240))
    discordLbl.Size = UDim2.new(1, -100, 1, 0)
    discordLbl.Position = UDim2.new(0, 10, 0, 0)
    discordLbl.ZIndex = 4

    local copyBtn = Instance.new("TextButton", discordRow)
    copyBtn.Size = UDim2.new(0, 80, 0, 22)
    copyBtn.Position = UDim2.new(1, -88, 0.5, 0)
    copyBtn.AnchorPoint = Vector2.new(0, 0.5)
    copyBtn.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
    copyBtn.Text = "Copy"
    copyBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
    copyBtn.TextSize = IS_MOBILE and 12 or 13
    ...
    corner(copyBtn, 6)
    copyBtn.MouseEnter:Connect(function() Tween:Create(copyBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(110, 120, 250)}):Play() end)
    copyBtn.MouseLeave:Connect(function() Tween:Create(copyBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(88, 101, 242)}):Play() end)
    ...
    infoFrame.Size = UDim2.new(1, 0, 0, IS_MOBILE and 100 or 110)
    ...
    local function makeInfoRow(txt, order)
        local l = label(infoFrame, txt, IS_MOBILE and 11 or 13, Color3.fromRGB(220, 230, 240))
        l.Size = UDim2.new(1, 0, 0, IS_MOBILE and 16 or 18)
        l.LayoutOrder = order
        l.ZIndex = 4
        return l
    end
...
do
    ...
    local kickScroll = makeGroup(p, "Kick", 3)
    local kickMethodDD
    makeSmallToggle(kickScroll, "Kick", false, function(v)
        S.kick = v
        if v and S.target and S.target ~= "" then
            S.kickTask = task.spawn(function()
                if S.kickMethod == "Kick 1" then
                    kick1(S.target)
                end
            end)
        elseif v then
            ...
            if S.kickTask then pcall(task.cancel, S.kickTask); S.kickTask = nil end
    ...
    kickMethodDD = makeSmallDropdown(kickScroll, "Method", {"Kick 1"}, "Kick 1", function(v)
        S.kickMethod = v
        -- если Kick включён — перезапускаем с новым методом
        if S.kick and S.target and S.target ~= "" then
            if S.kickTask then pcall(task.cancel, S.kickTask); S.kickTask = nil end
            S.kickTask = task.spawn(function()
                if S.kickMethod == "Kick 1" then kick1(S.target) end
            end)
        end
    end, 2)

    local ragScroll = makeGroup(p, "Ragdoll", 4)
    makeSmallToggle(ragScroll, "Ragdoll", false, function(v)
        S.rogdoll = v
        if v and S.target and S.target ~= "" then
            S.rogdollTask = task.spawn(function()
                if S.rogdollMethod == "Pallet Rogdoll" then palletRagdoll(S.target)
                elseif S.rogdollMethod == "Hit Rogdoll" then hitRagdoll(S.target)
                elseif S.rogdollMethod == "Snowball Rogdoll" then snowballRagdoll(S.target) end
            end)
        elseif v then S.rogdoll = false
        else
            if S.rogdollTask then pcall(task.cancel, S.rogdollTask); S.rogdollTask = nil end
            -- очищаем возможные BodyPosition
            local t = Players:FindFirstChild(S.target or "")
            if t and t.Character then
                local tr = t.Character:FindFirstChild("HumanoidRootPart")
                if tr then
    ...
    local ragMethodDD = makeSmallDropdown(ragScroll, "Method", {"Pallet Rogdoll", "Hit Rogdoll", "Snowball Rogdoll"}, "Pallet Rogdoll", function(v)
        S.rogdollMethod = v
        if S.rogdoll and S.target and S.target ~= "" then
            if S.rogdollTask then pcall(task.cancel, S.rogdollTask); S.rogdollTask = nil end
            S.rogdollTask = task.spawn(function()
                if S.rogdollMethod == "Pallet Rogdoll" then palletRagdoll(S.target)
                elseif S.rogdollMethod == "Hit Rogdoll" then hitRagdoll(S.target)
    ...
    local xzScroll = makeGroup(p, "XZ", 5)
    ...
    local traceScroll = makeGroup(p, "Trace", 6)
...
toggleBtn.Name = "ErisToggleBtn"
toggleBtn.AnchorPoint = Vector2.new(0, 0.5)
toggleBtn.Position = UDim2.new(0, 10, 0.5, 0)
toggleBtn.Size = UDim2.new(0, 56, 0, 56)
toggleBtn.BackgroundColor3 = Color3.fromRGB(10, 14, 20)
toggleBtn.BackgroundTransparency = 0
toggleBtn.Text = ""
toggleBtn.AutoButtonColor = false
toggleBtn.BorderSizePixel = 0
toggleBtn.ZIndex = 20000
corner(toggleBtn, 12)
stroke(toggleBtn, ACCENT, 0.2)

local toggleImg = Instance.new("ImageLabel", toggleBtn)
toggleImg.Size = UDim2.new(1, -4, 1, -4)
toggleImg.Position = UDim2.new(0.5, 0, 0.5, 0)
toggleImg.AnchorPoint = Vector2.new(0.5, 0.5)
toggleImg.BackgroundTransparency = 1
toggleImg.Image = BG_IMAGE
toggleImg.ScaleType = Enum.ScaleType.Crop
toggleImg.ImageTransparency = 0.15
toggleImg.ZIndex = 20001
corner(toggleImg, 10)
...
toggleBtn.MouseEnter:Connect(function()
    Tween:Create(toggleStroke, TweenInfo.new(0.15), {Transparency = 0}):Play()
end)
toggleBtn.MouseLeave:Connect(function()
    Tween:Create(toggleStroke, TweenInfo.new(0.15), {Transparency = 0.2}):Play()
end)
...
local function setMenu(open)
    ...
    if open then
        main.Visible = true
        main.Size = UDim2.new(0, IS_MOBILE and (MOBILE_W - 100) or (PC_W - 160), 0, IS_MOBILE and (MOBILE_H - 100) or (PC_H - 160))
        ...
        local t = Tween:Create(main, ti, {Size = UDim2.new(0, IS_MOBILE and (MOBILE_W - 100) or (PC_W - 160), 0, IS_MOBILE and (MOBILE_H - 100) or (PC_H - 160))})
        t:Play()
...
toggleBtn.MouseButton1Click:Connect(function()
    if isOpen then
        setMenu(false)
        if IS_MOBILE then toggleImg.ImageTransparency = 0.05 end
    else
        setMenu(true)
        if IS_MOBILE then toggleImg.ImageTransparency = 0.35 end
    end
end)
...
setMenu(true)
if IS_MOBILE then
    toggleImg.ImageTransparency = 0.35
else
    toggleImg.ImageTransparency = 0.15
end
    ...
    setMenu = function(open)
        ...
        if open then
            ...
            task.delay(0.4, function()
                if not isOpen then toggleBtn.Visible = true end
            end)
        end
    end
end
